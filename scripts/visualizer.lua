...
local was_enabled = false -- track whether we've actually changed the visualizer
...
local function visualizer_hook()
	...
	print(opts.name)
	if opts.name == "hauto" then
		if albumart == 0 and vtrack == 0 and atrack > 0 then
			opts.name = "showcqtbar"
		else
			opts.name = "off"
		end
	end
	print(opts.name, albumart, vtrack, atrack)
...
mp.add_key_binding(reverse_cycle_key, "reverse-cycle-visualizer", cycle_visualizer_reverse)