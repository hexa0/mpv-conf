...
-- https://github.com/diffficult/mpv_crop_script
...
function DisplayState:recalculate_bounds(forced)
    ...
    if new_state.video_w and new_state.video_h then
        ...
        if new_state.keepaspect then
            ...
            local video_left, video_right = self:_split_scaling(new_state.screen_w, scaled_w, new_state.video_zoom,
                new_state.video_align_x, new_state.video_pan_x)
            local video_top, video_bottom = self:_split_scaling(new_state.screen_h, scaled_h, new_state.video_zoom,
                new_state.video_align_y, new_state.video_pan_y)
            self.bounds = {
                left = video_left,
                right = video_right,
...
function ASSCropper:get_render_ass(dim_only)
    ...
    local guide_format = string.format("{\\3a&HFF&\\3a&H%02X&\\3c&H%02X%02X%02X&\\bord1\\shad0}", 128, line_color,
    ...
    if self.current_crop then
        if self.testing_crop then
            ...
            ass:append(fmt_key("ENTER", "Accept crop") ..
            " " .. fmt_key("ESC", "Cancel crop") .. '\\N' .. fmt_key("T", "Stop testing"))
            return ass.text
        end
        ...
        -- Dim overlay
        local format_dim = string.format("{\\bord0\\1a&H%02X&\\1c&H%02X%02X%02X&}", self.overlay_transparency,
            self.overlay_lightness, self.overlay_lightness, self.overlay_lightness)
        ass:pos(0, 0)
        ass:draw_start()
        ...
        local box_format = string.format("{\\1a&HFF&\\3a&H%02X&\\3c&H%02X%02X%02X&\\bord1}", 0, line_color, line_color,
            line_color)
        local handle_hilight_format = string.format("{\\1a&H%02X&\\3a&H%02X&\\3c&H%02X%02X%02X&\\bord0}", 230, 0,
            line_color, line_color, line_color)
        local handle_drag_format = string.format("{\\1a&H%02X&\\3a&H%02X&\\3c&H%02X%02X%02X&\\bord1}", 200, 0, line_color,
            line_color, line_color)
    ...
    if self.options.draw_help and not dim_only then
        ...
        lines = {
            ...
            " " ..
            fmt_key("ESC", "Cancel crop") .. " " .. fmt_key("D", "Autodetect crop") .. " " .. fmt_key("T", "Test crop"),
            fmt_key("SHIFT-Drag", "Constrain ratio") .. " " .. fmt_key("SHIFT-Arrow", "Nudge"),
            fmt_key("C", crosshair_txt .. " crosshair") ..
            " " .. fmt_key("X", "Cycle guides") .. " " .. fmt_key("Z", "Invert color"),
        }
...
if option_values.output_extension == "" then
    ...
    if not extension then
        msg.error("Unrecognized output format '" ..
        option_values.output_format .. "', unable to pick an extension! Bailing!")
